...
from ht15.core.assertions import assert_response_full, assert_response_code


@allure.suite("Tests for pet service")
@allure.sub_suite("GET")
class TestPetGet:
    @allure.title("Existing pet data can be successfully retrieved using GET /pet/{pet_id} endpoint")
    ...
    @allure.title("Error 404 is displayed on attempt to retrieve pet using invalid pet id")
    @pytest.mark.parametrize("pet_id, message, key_type, code",
    ...
    @allure.title("Error 405 is displayed on attempt to retrieve pet using empty pet id")
