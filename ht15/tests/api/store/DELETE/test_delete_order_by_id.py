...
from ht15.core.assertions import assert_response_code


@allure.suite("Tests for store service")
@allure.sub_suite("DELETE")
class TestOrderDelete:
    @allure.title("Existing order can be successfully deleted using DELETE /store/order/{order_id} endpoint")
    ...
    @allure.title("Error 404 is displayed on attempt to delete order by invalid order id")
    @pytest.mark.parametrize("order_id, message, key_type, code",
                             [(7777, "Order Not Found", "unknown", 404),
                              ('a', "java.lang.NumberFormatException: For input string: \"a\"", "unknown", 404),
                              ('!@#$', "java.lang.NumberFormatException: For input string: \"!@\"", "unknown", 404)])
    def test_delete_by_invalid_order_id(self, store_service, order_id, message, key_type, code):
        """
    ...
    @allure.title("Error 405 is displayed on attempt to delete order using empty order id")
