...
def assert_response_min(pet_data, body_req):
    ...
    assert pet_data["photoUrls"] == body_req["photoUrls"], "Unexpected 'photoUrls' value"
...
def assert_response_code(response, code=200):
    assert response.status_code == code, f'Expected code = {code}, got {response.status_code}'
...
def assert_user_list_created_response(user_data):
    assert user_data["code"] == 200, f"Expected 'code' == 200, Actual - {user_data["code"]}"
    assert user_data["type"] == "unknown", f"Expected 'type' == 'unknown', Actual - {user_data["type"]}"
    assert user_data["message"] == "ok", f"Expected 'message' == 'ok', Actual - {user_data["message"]}"
...
def assert_user_updated_response(user_data, body_req):
    ...
    assert user_data["message"] == str(
        body_req["id"]), f"Unexpected 'message' value, 'message' should be equal to user id"
