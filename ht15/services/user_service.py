...
from ht15.core.base_service import BaseService
...
class UserService(BaseService):
    ...
    def create_with_list(self, user_list):
        response = self.post(f"{self.BASE_URL}/createWithList", headers=self.headers, json=user_list)
        self._check_response(response)
        return response
    ...
    def update_user(self, user_data, username):
        response = self.put(f"{self.BASE_URL}/{username}", headers=self.headers, json=user_data)
        self._check_response(response)
        return response
